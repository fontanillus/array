...
function crearTamagotchi() {
    ...
    mostrarEstado("¡Tu mascota ha nacido!", "img/nacer.mp4", "Mascota nacida");
}
...
function mostrarEstado(mensajeTexto = "", imagenSrc = "", imagenAlt = "") {
    ...
    if (imagenSrc) {
        if (imagenSrc.endsWith('.mp4')) {
            imagenMascota.innerHTML = `<video src="${imagenSrc}" alt="${imagenAlt}" class="h-full w-full object-contain rounded-2xl" autoplay loop muted playsinline></video>`;
        } else {
...
function alimentar() {
    ...
    limitarValores();
    mostrarEstado("Has alimentado a tu Tamagotchi 🍎", "img/alimentar.mp4", "Mascota alimentada");
}
...
function jugar() {
    ...
    limitarValores();
    mostrarEstado("Jugaste con tu Tamagotchi 🎾", "img/jugar.mp4", "Mascota ha jugado");
}
...
function dormir() {
    ...
    limitarValores();
    mostrarEstado("Tu Tamagotchi ha dormido 😴", "img/dormir.mp4", "Mascota ha dormido");
}
...
function duchar() {
    ...
    limitarValores();
    mostrarEstado("Bañaste a tu Tamagotchi 🛁", "img/duchar.mp4", "Mascota se ha duchado");
}

function reprender() {
    tamagotchi.felicidad -= 3;
    limitarValores();
    mostrarEstado("Reprendiste a tu Tamagotchi 📛", "img/rependrer.mp4", "Mascota ha sido reprendida");
}

function acariciar() {
    tamagotchi.felicidad += 4;
    limitarValores();
    mostrarEstado("Acariciaste a tu Tamagotchi 💖", "img/acariciar.mp4", "Mascota ha sido acariciada");
}